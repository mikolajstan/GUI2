...
public class FloorControlPanel extends JPanel {
  ...
  private Dimension calculatePanelSize() {
    ...
    int width =
        cols * BUTTON_SIZE + (cols - 1) * BUTTON_SPACING + 20 + 4; // 20 for padding, 4 for border
    int height =
        rows * BUTTON_SIZE + (rows - 1) * BUTTON_SPACING + 20 + 4; // 20 for padding, 4 for border
    ...
    JButton button =
        new JButton(text) {
          ...
          public boolean contains(int x, int y) {
            ...
            int distance =
                (int) Math.sqrt((x - centerX) * (x - centerX) + (y - centerY) * (y - centerY));
  ...
  private void addComponents() {
    ...
    for (int floorNum = 1; floorNum <= numFloors; floorNum++) {
      ...
      int y =
          10 + (rows - 1 - row) * (BUTTON_SIZE + BUTTON_SPACING); // 10 = border padding, flip row
